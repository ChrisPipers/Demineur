...
TEST_CASE("test2 placeFlagPosition()")
{
    ...
    SECTION("create a Board by default 20*20 and put 3 flags at (-1,-1), (0,0), (19,19) and (20,20)")
    {
        ...
        REQUIRE_THROWS(b.placeFlagPosition(Position (-1,-1)));
    }
}
...
TEST_CASE("test1 deleteFlagPosition()")
{
    ...
    while(!findPos)
    {
        ...
        while(!findPos)
        {
            if ((b.getValPosition(Position(i,j)))==('b'))
            {
                findPos = true;
                tempPo = Position(i,j);
                //cout << "position avec b " << i << ", " << j << endl;
            }
            j++;
        }
        i++;
    }
    b.placeFlagPosition(tempPo);
    //b.displayBoard();
    b.deleteFlagPosition(tempPo);
    //b.displayBoard();
    SECTION("create a Board by default 10*10 and put 3 flags at (0,0), (5,5) and (19,19) and delete them")
    {

        REQUIRE(b.getValPosition(tempPo)=='b');
    }
}
...
TEST_CASE("test1 placeClearPosition()")
{
    ...
    b.placeClearPosition(Position (1,1));
    b.placeClearPosition(Position (5,5));
    b.placeClearPosition(Position (13,13));

    SECTION("create a Board by default 15*15 and put 3 clear position at (0,0), (5,5) and (19,19)")
    {
        REQUIRE(b.getValPosition(Position(1,1))=='c');
        REQUIRE(b.getValPosition(Position(5,5))=='c');
        REQUIRE(b.getValPosition(Position(13,13))=='c');
...
TEST_CASE("test1 placeIndicePosition()")
{
    ...
    b.placeIndicePosition(Position (9,9),3);
   // b.displayBoard();
    SECTION("create a Board by default 10*10 and put 3 indice 1, 2, 3 at (0,0), (5,5) and (19,19)")
    {
        REQUIRE((b.getValPosition(Position(0,0))=='1'));
        REQUIRE((b.getValPosition(Position(5,5))=='2'));
        REQUIRE((b.getValPosition(Position(9,9))=='3'));
    }
}
...
TEST_CASE("test3 placeIndicePosition()")
{
    Board b = Board(20,20);
    int choice = 1;
    Position posDepart = Position(1,1);
    ...
    b.placeIndicePosition(Position (0,0),1);
    b.placeIndicePosition(Position (5,5),2);
    b.placeIndicePosition(Position (19,19),8);

    SECTION("create a Board by default 20*20 and put 3 indice 1, 2, 8 at (0,0), (5,5) and (19,19)"
            "and 8 at (4,4) , 9 at (6,6), -1 at (12,10) and -9 at (0,19) ")
    {
        REQUIRE((char)b.getValPosition(Position(0,0))=='1');
        REQUIRE((char)b.getValPosition(Position(5,5))=='2');
        REQUIRE((char)b.getValPosition(Position(19,19))=='8');
...
TEST_CASE("test1 inBoard()")
{
    ...
    SECTION("test if in board with the position 0,0 , 5,5 and 9,9 in a board 10*10")
    {
        REQUIRE((b.inBoard(Position(0,0))==true));
        REQUIRE((b.inBoard(Position(5,5))==true));
        REQUIRE((b.inBoard(Position(9,9))==true));
    }
}

TEST_CASE("test2 inBoard()")
{
    Board b = Board(10,10);
    int choice = 1;
    Position posDepart = Position(1,1);
    ...
    SECTION("test if in board with the position 0,0 , 5,5 , 9,9 , 10,10 , -1,-1 in a board 10*10")
    {
        REQUIRE((b.inBoard(Position(0,0))==true));
        REQUIRE((b.inBoard(Position(5,5))==true));
        REQUIRE((b.inBoard(Position(9,9))==true));

        REQUIRE((b.inBoard(Position(10,10))==false));
        REQUIRE((b.inBoard(Position(-1,-1))==false));
    }
}
...
TEST_CASE("test1 revelantPosFirst()")
{
    ...
    SECTION("test revelant position first")
    {
        REQUIRE( ((b.getValPosition(posDepart)=='c')|| (isdigit(b.getValPosition(posDepart))) ) );
    }
}
...
TEST_CASE("test2 revelantPosFirst()")
{
    Board b = Board(20,20);
    int choice = 1;
    Position posDepart = Position(19,19);
...
TEST_CASE("test3 revelantPosFirst()")
{
    Board b = Board(20,20);
    int choice = 1;
    Position posDepart = Position(5,5);
...
TEST_CASE("test1 revelantPosRecursif()")
{
    // compliquer à tester
