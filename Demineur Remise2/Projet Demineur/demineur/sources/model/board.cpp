...
Board::Board (int width, int height)
{
    this->width_ = width;
    this->height_ = height;
    vector<vector<char>> board_(height);
    posBombs_.clear();
    posFlags_.clear();
    initBoard();
    notifyObserver();
...
{
    if( (inBoard(Position(position.getAbs(),position.getOrd())))
    ...
    {
        queueToVisit_.push(Position(position.getAbs(),position.getOrd()));
    }
}
...
void Board::updateboard()
{
    ...
    cout << endl << endl;
    displayBoardForPlayer();
    cout << "nombre de bombes : "<<nbMine_;
}

void Board::setIsInitilized()
{
    this->isInitilize_=true;
}
